...
def terminal(request):
    return Shell(width=request.param.width, os_name=request.param.os_name, indentation="    ")


@pytest.mark.parametrize(argnames="terminal, cmd", argvalues=clear_arg_grid, indirect=["terminal"])
def test_clear(capsys, terminal, cmd):
...
def test_print(capsys, terminal, paragraphs, alignment, expected):
    with mock.patch(target="engine.Shell.clear"):
...
@pytest.mark.parametrize(
    argnames="terminal, input", argvalues=["this is some text", ""], indirect=["terminal"]
)
